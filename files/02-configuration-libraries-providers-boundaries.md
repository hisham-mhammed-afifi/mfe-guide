...
> **Note:** TypeScript cannot resolve `'mfe_products/Routes'` until the first build generates `.d.ts` declaration files. If your IDE shows red squiggles on the `typeof import(...)` before you have ever run `nx build` or `nx serve`, this is expected. Running `npx nx serve shell` (which builds all remotes) generates the declaration files and resolves the error. During active development before the first build, you can suppress the error by replacing the generic with an explicit type: `loadRemote<{ remoteRoutes: Route[] }>('mfe_products/Routes')`.
...
Open the generated `libs/shared/models/src/index.ts` and replace its contents with:
...
Open the generated `libs/shared/data-access-auth/src/index.ts` and replace its contents with:
...
Each remote has its own `app.config.ts` used only when running standalone with `nx serve mfe_products`. Open the generated `apps/mfe_products/src/app/app.config.ts` and replace its contents with:
...
| `apps/mfe_products` | `["scope:products", "type:app"]` |
| `apps/mfe_orders` | `["scope:orders", "type:app"]` |
| `apps/mfe_account` | `["scope:account", "type:app"]` |
...
Open `eslint.config.mjs` at the workspace root. The generator places the `@nx/enforce-module-boundaries` rule inside a config object that looks like this (the `depConstraints` array is empty by default):
  ...
  rules: {
    '@nx/enforce-module-boundaries': [
      'error',
      {
        ...
        depConstraints: [],   // <-- empty by default
      },
    ],
  },
},
...
Replace the empty `depConstraints: []` with the array below:
