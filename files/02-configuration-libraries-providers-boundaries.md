...
> **Tip:** The route `path` does not need to match the remote name. Use user-friendly paths like `'products'` instead of `'mfe_products'`. The `loadRemote` string must still use the registered remote name.
