...
- **Node.js 18+** installed (20.x LTS recommended)
- **npm**, yarn, or pnpm
- **Angular CLI basics**: you know what `ng serve` and `ng generate component` do
- **Standalone components**: you have used `@Component({ standalone: true })` and `provideRouter()`
...
|---|---|---|
| Angular | 21.x | UI framework (standalone components, signals, new control flow) |
| Nx | 22.x | Monorepo build system (caching, generators, boundaries) |
| Webpack | 5.x | Bundler (required for Module Federation) |
| `@module-federation/enhanced` | 0.8+ | Module Federation runtime (maintained by Zack Jackson and ByteDance) |
...
- **Chapter 13: Testing Strategy**
  Unit tests, integration tests with Docker Compose, and contract tests with working code.
