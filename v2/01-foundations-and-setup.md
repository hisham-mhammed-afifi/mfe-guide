...
**Shared libraries** (like `@angular/core`, your `AuthService`, or a shared button component) are loaded once and reused by all applications. Module Federation handles this automatically so you do not get duplicate copies of Angular in the browser.
...
| **Dynamic** | Remote URLs are loaded from a JSON file at runtime | No (change the JSON file only) | Production, multi-environment |

**This guide uses Dynamic Federation throughout.** It is the recommended approach for any real project because it enables the "build once, deploy everywhere" pattern. You build the shell once, and swap the JSON file to point remotes at different URLs per environment (dev, staging, production).
...
> - [x] The shell's `app.routes.ts` has lazy routes that load each remote via `loadRemoteModule()`
